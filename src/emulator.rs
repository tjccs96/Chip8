...
impl Emulator {
    ...
    fn set_keycode(&mut self, code: Keycode, state: bool) {
        match code {
            Keycode::Num1 => {
                self.cpu.keypad.set(0x1, state);
            }
            Keycode::Num2 => {
                self.cpu.keypad.set(0x2, state);
            }
            Keycode::Num3 => {
                self.cpu.keypad.set(0x3, state);
            }
            Keycode::Num4 => {
                self.cpu.keypad.set(0xC, state);
            }
            Keycode::Q => {
                self.cpu.keypad.set(0x4, state);
            }
            Keycode::W => {
                self.cpu.keypad.set(0x5, state);
            }
            Keycode::E => {
                self.cpu.keypad.set(0x6, state);
            }
            Keycode::R => {
                self.cpu.keypad.set(0xD, state);
            }
            Keycode::A => {
                self.cpu.keypad.set(0x7, state);
            }
            Keycode::S => {
                self.cpu.keypad.set(0x8, state);
            }
            Keycode::D => {
                self.cpu.keypad.set(0x9, state);
            }
            Keycode::F => {
                self.cpu.keypad.set(0xE, state);
            }
            Keycode::Z => {
                self.cpu.keypad.set(0xA, state);
            }
            Keycode::X => {
                self.cpu.keypad.set(0x0, state);
            }
            Keycode::C => {
                self.cpu.keypad.set(0xB, state);
            }
            Keycode::V => {
                self.cpu.keypad.set(0xF, state);
            }
            _ => {}
        }
    }
            ...
            for event in event_pump.poll_iter() {
                match event {
                    ...
                    Event::KeyDown {
                        ...
                        ..
                    } => self_mut.set_keycode(keycode, true),
                    Event::KeyUp {
                        keycode: Some(keycode),
                        ..
                    } => self_mut.set_keycode(keycode, false),
                    _ => {}
                }
            }
